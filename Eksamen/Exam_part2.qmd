...
print(performance_table_report.to_markdown(index=False))
print("\n: Out-of-sample performance of the high-frequency, Ledoit-Wolf, and equal-weighted strategies. {#tbl-oos-performance}")
```
...
hf_vol = round(perf_lookup.loc["HF", "annualized_volatility_percent"], 2)
lw_vol = round(perf_lookup.loc["LW", "annualized_volatility_percent"], 2)
ew_vol = round(perf_lookup.loc["EW", "annualized_volatility_percent"], 2)

hf_sharpe = round(perf_lookup.loc["HF", "annualized_sharpe"], 3)
lw_sharpe = round(perf_lookup.loc["LW", "annualized_sharpe"], 3)
ew_sharpe = round(perf_lookup.loc["EW", "annualized_sharpe"], 3)

hf_minus_lw_sharpe = round(hf_sharpe - lw_sharpe, 3)
hf_minus_ew_sharpe = round(hf_sharpe - ew_sharpe, 3)
lw_minus_ew_sharpe = round(lw_sharpe - ew_sharpe, 3)
