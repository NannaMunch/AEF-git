...
Figure 1 plots the resulting universe size over time. After filtering and subsampling, the universe contains on average **`{python} f"{avg_n:.0f}"`** stocks per month, with a minimum of `{python} f"{n_per_month.min()}"` and a maximum of `{python} f"{n_per_month.max()}"`. The universe is visibly larger during the late 1990s, consistent with the broader expansion of public equity markets at the time, and contracts again after the dot-com crash and subsequent listings decline.
...
We estimate $\hat\Sigma_t$ at each rebalancing date using a rolling window of the preceding 36 months of returns, requiring that a stock have at least 70% non-missing observations within the window to be included; any remaining missing values are filled with the cross-sectional mean for that month. A factor model based on the Fama-French factors was considered as an alternative, but we opted for the simpler rolling-window approach, which keeps the assignment self-contained and avoids introducing an additional data source whose choice of factors would itself require justification.

**3a.** On top of the raw sample covariance $\hat\Sigma_t$, we apply Ledoit-Wolf analytical linear shrinkage, implemented via `sklearn.covariance.LedoitWolf`, to obtain a second estimator $\hat\Sigma_t^{LW}$. This shrinks the sample covariance towards a constant-correlation target, following Ledoit and Wolf (2003, 2004), with the shrinkage intensity chosen automatically to minimise expected estimation error.
...
(a) **naive diversification**, holding equal weights $1/N$ and rebalancing monthly back to equal weight;
...
```{python}
def compute_turnover(w_new, w_prev, ret):
    gross = w_prev * (1 + ret)
    denom = gross.sum()
    if denom <= 0:
        return np.nan
    w_drifted = gross / denom
    return np.sum(np.abs(w_new - w_drifted))
```
...
for t_idx, date in enumerate(oos_dates):
    ...
    if ml_mu is not None:
        aligned = ml_mu.reindex(permnos_w)                 # <-- test
        ml_months += 1                                     # <-- test
        ml_cov.append(aligned.notna().mean())              # <-- test
    ...
    def get_w_prev(strategy, permnos):
        ...
        prev_date_idx = list(oos_dates).index(date) - 1
        if prev_date_idx >= 0:
    ...
    for strat, w_new, w0 in [
        ...
        to       = compute_turnover(w_new, w0, realized_ret)
        results[strat]["ret"].append(port_ret)
        results[strat]["turnover"].append(to)
        results[strat]["dates"].append(date)
        w_prev[strat] = (w_new, permnos_w)

if ml_months:                                              # <-- test (fjern før aflevering)
    print(f"ML predictions used in {ml_months}/{len(oos_dates)} months; "
          f"avg cross-sectional coverage {np.mean(ml_cov)*100:.0f}%")
else:
    print("WARNING: ML predictions never used — fell back to historical mean every month")
```
...
ax.legend(fontsize=8)
plt.tight_layout()
plt.savefig("fig_turnover.pdf", bbox_inches="tight")
plt.show()
```
...
axes[1].legend()
plt.tight_layout()
plt.savefig("fig_cov_comparison.pdf", bbox_inches="tight")
plt.show()
```
