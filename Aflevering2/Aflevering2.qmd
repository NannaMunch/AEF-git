...
`retvol`, and valuation/fundamentals: `bm`, `ep`, `roaq`, `invest`. For the macroeconomic state variables from @Goyal.2008 library, we follow @Gu.2020 Table 4 and include aggregate book-to-market (`bm`), the Treasury bill rate (`tbl`), the default yield spread (`dfy`), and the term spread (`tms`). These variables capture aggregate valuation, interest rate conditions, credit risk, and the slope of the yield curve.
...
@tbl-summary-statistics reports summary statistics for the selected predictors. Stock characteristics are approximately centered around zero and bounded close to $[-1,1]$, while macro variables vary at the monthly aggregate level. 
...
### Exercise 3f: Model comparison
For the selected hyperparameter specifications, Elastic Net is the preferred model from an investor perspective. In the test period, Elastic Net generates an annualized mean return of `{python} f"{en_test['ann_mean']*100:.2f}"`%, a Sharpe ratio of `{python} f"{en_test['sharpe']:.2f}"`, and a CAPM alpha of `{python} f"{en_test['alpha']*100:.2f}"`%. Random Forest performs poorly out-of-sample, with an annualized mean return of `{python} f"{rf_test['ann_mean']*100:.2f}"`%, a Sharpe ratio of `{python} f"{rf_test['sharpe']:.2f}"`, and a CAPM alpha of `{python} f"{rf_test['alpha']*100:.2f}"`%.
