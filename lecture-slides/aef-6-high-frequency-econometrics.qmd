...
ggplot(order_book, aes(x = price, y = depth, fill = side)) +
  ...
  annotate("segment", x = 4, xend = 4, y = -2.5, yend = -0.1,
           arrow = arrow(length = unit(0.15, "cm"), ends = "last"), linewidth = 0.4) +
  annotate("text", x = 3.55, y = -3, label = "bid-price", size = 3) +
  # ask-price arrow & label
  annotate("segment", x = 5, xend = 5, y = -2, yend = -0.1,
           arrow = arrow(length = unit(0.15, "cm"), ends = "last"), linewidth = 0.4) +
  annotate("text", x = 5.45, y = -2.5, label = "ask-price", size = 3) +
  # mid-price arrow & label
  annotate("segment", x = 4.5, xend = 4.5, y = 5.5, yend = 0.1,
           arrow = arrow(length = unit(0.15, "cm"), ends = "last"), linewidth = 0.4) +
  annotate("text", x = 4.5, y = 6, label = "mid-price", size = 3) +
  # spread bracket
  annotate("segment", x = 4.05, xend = 4.95, y = -1.5, yend = -1.5, linewidth = 0.4) +
  annotate("segment", x = 4.05, xend = 4.05, y = -1.2, yend = -1.8, linewidth = 0.4) +
  annotate("segment", x = 4.95, xend = 4.95, y = -1.2, yend = -1.8, linewidth = 0.4) +
  annotate("text", x = 4.5, y = -2, label = "spread", size = 3) +
  # Buy limit order label
  annotate("segment", x = 1.5, xend = 1.1, y = -8.5, yend = -7.2,
           arrow = arrow(length = unit(0.15, "cm"), ends = "last"), linewidth = 0.4) +
  annotate("text", x = 2, y = -9, label = "Buy limit order", size = 3) +
  # Sell limit order label
  annotate("segment", x = 6, xend = 6, y = 8, yend = 7.2,
           arrow = arrow(length = unit(0.15, "cm"), ends = "last"), linewidth = 0.4) +
  annotate("text", x = 6, y = 8.6, label = "Sell limit order", size = 3) +
  # Side labels
  annotate("text", x = 1.5, y = -9.5, label = "Bid Side", fontface = "bold", size = 3.5) +
  annotate("text", x = 7.5, y = 9.5, label = "Ask Side", fontface = "bold", size = 3.5) +
...
- As a member of KU, you can retrieve data from [Lobster](www.lobsterdata.com)
...
- Set $U_{n,i} = \Delta X_{t_n,i}/(\sigma\Delta t_n^{1/2}) \sim N\left(\frac{\mu}{\sigma}\Delta t_n^{1/2}, 1\right)$ and define the sample mean $\bar U_n = \frac{1}{n}\sum\limits_{i = 0}^{n-1} U_{n,i}$
...
- As long as there are no jumps, $IV(0, 1) = RV^{(n)} := \sum\limits_{i=0}^{n-1}r_{i,n}^2$
...
```{r}
read_csv("../data-r/rv-data.csv", show_col_types = FALSE) |> ggplot(aes(x = Date, y = `Realized Variance (1-min)`))+geom_line() + theme_bw()
```
...
- Noise bias adjustment possible (Aït-Sahalia et al., 2005)
...
- Definition: Refresh time for $t\in[0,1]$. Define the first refresh time as $\tau_1 = \max\left(t_1^{(1)}, \ldots, t_1^{(N)}\right)$ and then iteratively
$$\tau_{j+1} = \max\left(t_{N_{\tau_j^{(1)}+1}}^{(1)}, \ldots, t_{N_{\tau_j^{(N)}+1}}^{(N)}\right) $$
