...
fit_polynomial <- function(degree, data = train_set) {
  formula <- as.formula(paste("returns ~ poly(time, ", degree, ")", sep = ""))
  model <- lm(formula, data)
  return(model)
}

results <- tibble(degree = c(1, 2, 5, 10, 19)) |> 
  mutate(lm = map(degree, fit_polynomial),
         lm_train = map(lm, predict, newdata = train_set),
         lm_test = map(lm, predict, newdata = test_set),
...
ggplot(data |> mutate(time = as.Date(time)), 
  ...
  geom_point() +
  coord_cartesian(
  ylim = c(min(data$returns), max(data$returns))
) +
